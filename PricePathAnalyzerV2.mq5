...
//--- extra toggle lines (Spike / chNL / Range) horizontal line pair
input bool            SpikeEnabled  = true;   // Enable Spike horizontal-lines button
input color           SpikeTopColor = clrGreen; // Spike top line color
input color           SpikeBotColor = clrRed;   // Spike bottom line color
input int             SpikeLineWidth = 2;      // Spike line thickness

input bool            NLEnabled     = true;    // Enable chNL horizontal-lines button
input color           NLTopColor    = clrGreen; // chNL top line color
input color           NLBotColor    = clrRed;   // chNL bottom line color
input int             NLLineWidth   = 2;        // chNL line thickness

input bool            RangeEnabled  = true;   // Enable Range horizontal-lines button
input color           RangeTopColor = clrGreen;// Range top line color
input color           RangeBotColor = clrRed;  // Range bottom line color
input int             RangeLineWidth = 2;     // Range line thickness
...
#define O_DRAG     PREFIX "DragBtn"

//--- extra toggle buttons (channel / range lines)
#define O_BTN_S    PREFIX "Btn_Spike"
#define O_BTN_N    PREFIX "Btn_NL"
#define O_BTN_R    PREFIX "Btn_Range"
#define O_HL_1      PREFIX "ToggleLine1"
#define O_HL_2      PREFIX "ToggleLine2"
#define O_HL_3      PREFIX "ToggleLine3"
#define O_HL_4      PREFIX "ToggleLine4"
#define O_HL_5      PREFIX "ToggleLine5"
#define O_HL_6      PREFIX "ToggleLine6"
...
//--- extra toggle button states (draw horizontal line pairs)
bool SpikeLinesOn = false;
bool NLLinesOn    = false;
bool RangeLinesOn = false;
      ...
      if(sparam == O_BTN_L)
        {
         ...
         if(sparam == O_BTN_C)
           {
            UpdateStatus("Calculate is disabled.\nUse the Path button to open the table.");
           }
         else
            if(sparam == O_BTN_P)
              {
...
void SetLinesVisible(bool visible)
  {
   ...
   //--- the T-line projection follows the vertical lines visibility
   SetObjTimeframes(PREFIX "TLine", visible);
   SetObjTimeframes(PREFIX "TLabel", visible);
   SetObjTimeframes(PREFIX "TInfo", visible);
   ChartRedraw();
  }
...
//--- background (draggable, with border) - reuse existing object if present
   RegisterChild(O_BG, 0, 0);
   bool freshPanel = (ObjectFind(0, O_BG) < 0);
   if(freshPanel)
     {
      //--- new panel: place at right side, middle of chart
      int cw = (int)ChartGetInteger(0, CHART_WIDTH_IN_PIXELS);
   ...
   ObjectSetInteger(0, O_BG, OBJPROP_BGCOLOR, C'5,10,40');
   ObjectSetInteger(0, O_BG, OBJPROP_BORDER_COLOR, clrWhite);
   ObjectSetInteger(0, O_BG, OBJPROP_BORDER_TYPE, BORDER_FLAT);
   ObjectSetInteger(0, O_BG, OBJPROP_WIDTH, 3);
...
//--- dedicated drag handle (grab & drag to move the whole panel)
//    main buttons below stay the same - this handle is only for moving
   RegisterChild(O_DRAG, PN_W - 68, 2);
   EnsureObject(O_DRAG, OBJ_RECTANGLE_LABEL);
   ObjectSetInteger(0, O_DRAG, OBJPROP_CORNER, CORNER_LEFT_UPPER);
   ObjectSetInteger(0, O_DRAG, OBJPROP_XDISTANCE, PanelX + PN_W - 68);
   ObjectSetInteger(0, O_DRAG, OBJPROP_YDISTANCE, PanelY + 2);
   ObjectSetInteger(0, O_DRAG, OBJPROP_XSIZE, 64);
   ObjectSetInteger(0, O_DRAG, OBJPROP_YSIZE, 20);
   ObjectSetInteger(0, O_DRAG, OBJPROP_BGCOLOR, C'5,10,40');
   ObjectSetInteger(0, O_DRAG, OBJPROP_BORDER_COLOR, clrWhite);
   ObjectSetInteger(0, O_DRAG, OBJPROP_BORDER_TYPE, BORDER_FLAT);
   ObjectSetInteger(0, O_DRAG, OBJPROP_WIDTH, 1);
   ObjectSetString(0, O_DRAG, OBJPROP_TEXT, "Drag");
   ObjectSetInteger(0, O_DRAG, OBJPROP_COLOR, clrWhite);
   ObjectSetInteger(0, O_DRAG, OBJPROP_FONTSIZE, 7);
   ObjectSetString(0, O_DRAG, OBJPROP_FONT, "Arial");
   ObjectSetInteger(0, O_DRAG, OBJPROP_BACK, false);
   ObjectSetInteger(0, O_DRAG, OBJPROP_SELECTABLE, true);
   ObjectSetInteger(0, O_DRAG, OBJPROP_SELECTED, false);
   ObjectSetInteger(0, O_DRAG, OBJPROP_ZORDER, 102);
   ObjectSetInteger(0, O_DRAG, OBJPROP_TIMEFRAMES, ALL_TF);
...
//--- buttons: 1 column x 7 rows
   ...
   RegisterChild(O_BTN_S, 12, 176);
   CreateButton(O_BTN_S, PanelX + 12, PanelY + 176, 180, 24, "Spike: OFF", C'60,60,60');

   RegisterChild(O_BTN_N, 12, 204);
   CreateButton(O_BTN_N, PanelX + 12, PanelY + 204, 180, 24, "chNL: OFF", C'60,60,60');

   RegisterChild(O_BTN_R, 12, 232);
   CreateButton(O_BTN_R, PanelX + 12, PanelY + 232, 180, 24, "Range: OFF", C'60,60,60');
...
//+------------------------------------------------------------------+
//+------------------------------------------------------------------+
//| Draw a pair of horizontal lines (Spike / chNL / Range)           |
//+------------------------------------------------------------------+
void DrawTogglePair(string line1, string line2, double price1, double price2,
                    color topColor, color botColor, int width)
  {
   ObjectDelete(0, line1);
   ObjectDelete(0, line2);

   datetime tF = Sel.StartTime, tL = Sel.EndTime;
   if(!Sel.HasSelection || tL <= tF)
     {
      int bars = iBars(_Symbol, _Period);
      if(bars <= 0)
         return;
      tF = iTime(_Symbol, _Period, bars - 1);
      tL = iTime(_Symbol, _Period, 0);
     }
   if(tL <= tF)
      return;

   if(!ObjectCreate(0, line1, OBJ_TREND, 0, tF, price1, tL, price1))
      return;
   ObjectSetInteger(0, line1, OBJPROP_COLOR, topColor);
   ObjectSetInteger(0, line1, OBJPROP_WIDTH, MathMax(1, width));
   ObjectSetInteger(0, line1, OBJPROP_STYLE, STYLE_SOLID);
   ObjectSetInteger(0, line1, OBJPROP_SELECTABLE, false);
   ObjectSetInteger(0, line1, OBJPROP_TIMEFRAMES, ALL_TF);

   if(!ObjectCreate(0, line2, OBJ_TREND, 0, tF, price2, tL, price2))
      return;
   ObjectSetInteger(0, line2, OBJPROP_COLOR, botColor);
   ObjectSetInteger(0, line2, OBJPROP_WIDTH, MathMax(1, width));
   ObjectSetInteger(0, line2, OBJPROP_STYLE, STYLE_SOLID);
   ObjectSetInteger(0, line2, OBJPROP_SELECTABLE, false);
   ObjectSetInteger(0, line2, OBJPROP_TIMEFRAMES, ALL_TF);

   ChartRedraw();
  }

//+------------------------------------------------------------------+
//| Get high/low price inside the selected range                     |
//+------------------------------------------------------------------+
bool GetRangeHighLow(double &hi, double &lo)
  {
   int bars = iBars(_Symbol, _Period);
   if(bars <= 0)
      return(false);
   ...
   for(int i = bars - 1; i >= 0; i--)
     {
      ...
      if(bt < Sel.StartTime || bt > Sel.EndTime)
         continue;
      double hi2 = iHigh(_Symbol, _Period, i);
      double lo2 = iLow(_Symbol, _Period, i);
      if(!found)
        {
         h = hi2;
         l = lo2;
...
//+------------------------------------------------------------------+
//| Toggle Spike horizontal lines                                    |
//+------------------------------------------------------------------+
//+------------------------------------------------------------------+
//| Turn off ALL three extra line pairs (Spike / chNL / Range)       |
//| Makes the three buttons mutually exclusive (only one active)     |
//+------------------------------------------------------------------+
void TurnOffExtraLines()
  {
   if(SpikeLinesOn)
     {
      ObjectDelete(0, O_HL_1);
      ObjectDelete(0, O_HL_2);
      SpikeLinesOn = false;
      ObjectSetString(0, O_BTN_S, OBJPROP_TEXT, "Spike: OFF");
      ObjectSetInteger(0, O_BTN_S, OBJPROP_BGCOLOR, C'60,60,60');
     }
   if(NLLinesOn)
     {
      ObjectDelete(0, O_HL_3);
      ObjectDelete(0, O_HL_4);
      NLLinesOn = false;
      ObjectSetString(0, O_BTN_N, OBJPROP_TEXT, "chNL: OFF");
      ObjectSetInteger(0, O_BTN_N, OBJPROP_BGCOLOR, C'60,60,60');
     }
   if(RangeLinesOn)
     {
      ObjectDelete(0, O_HL_5);
      ObjectDelete(0, O_HL_6);
      RangeLinesOn = false;
      ObjectSetString(0, O_BTN_R, OBJPROP_TEXT, "Range: OFF");
      ObjectSetInteger(0, O_BTN_R, OBJPROP_BGCOLOR, C'60,60,60');
     }
   ChartRedraw();
  }

void ToggleSpikeLines()
  {
   if(!SpikeLinesOn)
     {
      double hi, lo;
      TurnOffExtraLines();
      if(GetRangeHighLow(hi, lo))
         DrawTogglePair(O_HL_1, O_HL_2, hi, lo, SpikeTopColor, SpikeBotColor, SpikeLineWidth);
      SpikeLinesOn = true;
      ObjectSetString(0, O_BTN_S, OBJPROP_TEXT, "Spike: ON");
      ObjectSetInteger(0, O_BTN_S, OBJPROP_BGCOLOR, C'20,80,40');
     }
   else
     {
      ObjectDelete(0, O_HL_1);
   ...
   if(!NLLinesOn)
     {
      ...
      TurnOffExtraLines();
      if(GetRangeHighLow(hi, lo))
         DrawTogglePair(O_HL_3, O_HL_4, hi, lo, NLTopColor, NLBotColor, NLLineWidth);
      NLLinesOn = true;
      ObjectSetString(0, O_BTN_N, OBJPROP_TEXT, "chNL: ON");
      ObjectSetInteger(0, O_BTN_N, OBJPROP_BGCOLOR, C'20,80,40');
     }
   else
     {
      ObjectDelete(0, O_HL_3);
...
//+------------------------------------------------------------------+
//| Toggle Range horizontal lines                                    |
//+------------------------------------------------------------------+
void ToggleRangeLines()
  {
   if(!RangeLinesOn)
     {
      double hi, lo;
      TurnOffExtraLines();
      if(GetRangeHighLow(hi, lo))
         DrawTogglePair(O_HL_5, O_HL_6, hi, lo, RangeTopColor, RangeBotColor, RangeLineWidth);
      RangeLinesOn = true;
      ObjectSetString(0, O_BTN_R, OBJPROP_TEXT, "Range: ON");
      ObjectSetInteger(0, O_BTN_R, OBJPROP_BGCOLOR, C'40,80,40');
     }
   else
     {
      ObjectDelete(0, O_HL_5);
